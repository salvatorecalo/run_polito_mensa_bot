...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None) -> None:
    ...
    if not user or not user.selected_canteen_ids:
        text = "⚠️ Non sei iscritto a nessuna mensa. Usa '📍 GESTISCI MENSE' per iniziare."
        translated = await translate_text(text, language)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None):
    if not session or not update.callback_query: return
    ...
    if data == "menu":
        ...
        c_id = data.split("_")[-1]
        if c_id.isdigit():
            await handle_canteen_toggle(update, context, canteen_id=int(c_id))
...
async def show_canteen_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None):
    ...
    if not user or not all_canteens: return
    
    keyboard = []
    for c in all_canteens:
        status = "✅" if c.id in user.selected_canteen_ids else "❌"
        keyboard.append([InlineKeyboardButton(f"{status} {c.name}", callback_data=f"toggle_canteen_{c.id}")])
    ...
    if update.callback_query:
        await update.callback_query.edit_message_text(translated, reply_markup=InlineKeyboardMarkup(keyboard))
    elif update.effective_message:
        await update.effective_message.reply_text(translated, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_language_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None):
    ...
    common_langs = {'it': 'Italiano 🇮🇹', 'en': 'English 🇬🇧', 'es': 'Español 🇪🇸', 'fr': 'Français 🇫🇷', 'de': 'Deutsch 🇩🇪'}
    keyboard = [[InlineKeyboardButton(name, callback_data=f"lang_{code}")] for code, name in common_langs.items()]
    keyboard.append([InlineKeyboardButton("🔙 INDIETRO", callback_data="start_back")])
...
# --- Admin Commands (Mantengono logica testuale per sicurezza) ---
...
async def refresh_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None):
    """Forza l'aggiornamento dei menu (Solo Admin)"""
    if not update.effective_user or update.effective_user.id not in ADMIN_IDS or not update.effective_message:
        return
    
    status_msg = await update.effective_message.reply_text("🔄 Avvio aggiornamento menu...\nQuesta operazione potrebbe richiedere qualche minuto.")
    
    try:
        from services.scraper_service import fetch_and_store_menus
        await fetch_and_store_menus()
        await status_msg.edit_text("✅ Menu aggiornati con successo!\nUsa /menu per visualizzare i nuovi dati.")
    except Exception as e:
...
async def add_canteen(update: Update, context: ContextTypes.DEFAULT_TYPE, session=None):
    ...
    if not session or not update.effective_user or update.effective_user.id not in ADMIN_IDS or not update.effective_message:
        return
